...
export const initWritersPage = () => {
  const page = document.querySelector('.cast-page-main');
  if (!page) return;

  const searchInput = document.querySelector('[data-writers-search]');
  const sortSelect = document.querySelector('[data-writers-sort]');
  const alpha = document.querySelector('[data-writers-alpha]');
  const grid = document.querySelector('[data-writers-grid]');
  const pagination = document.querySelector('[data-writers-pagination]');
  const total = document.querySelector('[data-writers-total]');
  const status = document.querySelector('[data-writers-status]');
  const detail = document.querySelector('[data-writers-detail]');
  ...
  const state = {
    ...
    selected: parseInt(params.get('writer') || '', 10) || null,
    pages: 0,
  };
  ...
  const syncUrl = ({ replace = false } = {}) => {
    ...
    if (state.selected) next.set('writer', String(state.selected));
    ...
    detail.innerHTML = `
      ...
      <section class="cast-episode-section">
        <div class="cast-episode-heading">
          ...
          <a href="/episodes.html?writer=${encodeURIComponent(name)}">View all episodes →</a>
        </div>
        <div class="cast-episode-list">
          ${episodes.length ? episodes.map((episode) => `
            <a class="cast-episode-row" href="/episodes.html?writer=${encodeURIComponent(name)}&episode=${episode.episode_number}">
  ...
  const loadDetail = async (id) => {
    ...
    try {
      const [person, episodes] = await Promise.all([
        fetchJson(`/writers/${id}`),
        fetchJson(`/writers/${id}/episodes?page=1&limit=5&sort=broadcast_date&order=asc`),
      ]);
      renderDetail(person, episodes);
    } catch (error) {
      console.error('Unable to load writer:', error);
      detail.innerHTML = '<div class="cast-detail-empty">Writer details are temporarily unavailable.</div>';
    } finally {
      detail.removeAttribute('aria-busy');
    }
  };

  const selectWriter = (id, { updateUrl = true } = {}) => {
    state.selected = id;
    updateSelectedCard();
  ...
  const loadWriters = async () => {
    status.hidden = false;
    status.textContent = 'Loading writers…';
    ...
    try {
      const payload = await fetchJson(`/writers?${query}`);
      const people = payload.data || [];
      state.page = payload.pagination.page;
      state.pages = payload.pagination.pages;
      total.textContent = `${payload.pagination.total.toLocaleString('en-US')} writers`;

      if (!people.length) {
        grid.replaceChildren();
        pagination.replaceChildren();
        state.selected = null;
        detail.innerHTML = '<div class="cast-detail-empty">No writer matches the current filters.</div>';
        status.textContent = 'No writers found.';
